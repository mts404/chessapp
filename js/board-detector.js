...
class BoardDetector {
  ...
  analyze(img) {
    ...
    var board = this._findBoard(data, w, h);
    if (!board) return null;

    var sqColors = this._findSquareColors(data, w, board);
    if (!sqColors) return null;

    var grid = this._analyzeGrid(data, w, board, sqColors);
  ...
  _findBoard(data, imgW, imgH) {
    // Phase 1: row variance → approximate vertical extent
    var rowVar = new Float32Array(imgH);
    for (var y = 0; y < imgH; y++) {
      var sum = 0, sumSq = 0, n = 0;
      for (var x = 0; x < imgW; x += 3) {
        var v = this._grayAt(data, imgW, x, y);
        sum += v; sumSq += v * v; n++;
      }
      var mean = sum / n;
    ...
    var rowThresh = this._maxOf(rowVar) * 0.2;
    var rowRun = this._longestRun(rowVar, rowThresh, imgH);
    if (!rowRun || rowRun.len < imgH * 0.2) return null;

    // Phase 2: column variance within those rows → horizontal extent
    var colVar = new Float32Array(imgW);
    for (var x2 = 0; x2 < imgW; x2++) {
      var s = 0, s2 = 0, cn = 0;
      for (var y2 = rowRun.start; y2 < rowRun.start + rowRun.len; y2 += 3) {
        var val = this._grayAt(data, imgW, x2, y2);
        s += val; s2 += val * val; cn++;
      }
      var m = s / cn;
      colVar[x2] = s2 / cn - m * m;
    ...
    var colThresh = this._maxOf(colVar) * 0.2;
    var colRun = this._longestRun(colVar, colThresh, imgW);
    if (!colRun || colRun.len < imgW * 0.2) return null;

    // Board is square: use the smaller dimension
    ...
    // Phase 3: refine position with checkerboard scoring
    return this._refineBoard(data, imgW, imgH, approxX, approxY, approxSize);
  }
  ...
  _longestRun(arr, threshold, len) {
    var best = null, start = 0, inRun = false;
    for (var i = 0; i < len; i++) {
      if (arr[i] > threshold) {
        if (!inRun) { start = i; inRun = true; }
      } else if (inRun) {
        var runLen = i - start;
        if (!best || runLen > best.len) best = { start: start, len: runLen };
        inRun = false;
      }
    }
    if (inRun) {
      var rl = len - start;
      if (!best || rl > best.len) best = { start: start, len: rl };
    }
    return best;
  }

  _refineBoard(data, imgW, imgH, aX, aY, aSize) {
    var sqEst = aSize / 8;
    var step = Math.max(1, Math.round(sqEst * 0.1));
    var range = Math.round(sqEst * 0.7);
    ...
    for (var ds = -range; ds <= range; ds += step) {
      ...
      for (var dx = -range; dx <= range; dx += step) {
        for (var dy = -range; dy <= range; dy += step) {
          ...
          var score = this._checkerboardScore(data, imgW, tx, ty, trySize);
          if (score > bestScore) {
            bestScore = score;
    ...
    return (bestScore >= 1.5) ? bestRect : null;
  }

  _checkerboardScore(data, imgW, bx, by, bSize) {
    var sqSize = bSize / 8;
    var margin = sqSize * 0.12;
    ...
    for (var r = 0; r < 8; r++) {
      for (var c = 0; c < 8; c++) {
        ...
        var corners = [
          this._grayAt(data, imgW, sx + margin, sy + margin),
          this._grayAt(data, imgW, sx + sqSize - margin, sy + margin),
          this._grayAt(data, imgW, sx + margin, sy + sqSize - margin),
          this._grayAt(data, imgW, sx + sqSize - margin, sy + sqSize - margin),
        ];
        corners.sort(function (a, b) { return a - b; });
        var med = (corners[1] + corners[2]) / 2;

        if (isLight) lightVals.push(med);
        else darkVals.push(med);
      }
    }
  ...
  _findSquareColors(data, imgW, board) {
    ...
    for (var r = 0; r < 8; r++) {
      for (var c = 0; c < 8; c++) {
        ...
        var pts = [
          [sx + margin, sy + margin],
          [sx + sqSize - margin, sy + margin],
          [sx + margin, sy + sqSize - margin],
          [sx + sqSize - margin, sy + sqSize - margin],
        ];
        for (var p = 0; p < pts.length; p++) {
          var color = this._rgbAt(data, imgW, pts[p][0], pts[p][1]);
  ...
  _analyzeGrid(data, imgW, board, sqColors) {
    var sqSize = board.w / 8;
    var grid = [];
    ...
    for (var r = 0; r < 8; r++) {
      ...
      for (var c = 0; c < 8; c++) {
        ...
        var sx = board.x + c * sqSize;
        var sy = board.y + r * sqSize;
        grid[r][c] = this._analyzeSquare(data, imgW, sx, sy, sqSize, bgColor);
      }
    }
  ...
  _analyzeSquare(data, imgW, sqX, sqY, sqSize, bgColor) {
    ...
    for (var y = innerT; y < innerB; y += step) {
      for (var x = innerL; x < innerR; x += step) {
        var color = this._rgbAt(data, imgW, x, y);
